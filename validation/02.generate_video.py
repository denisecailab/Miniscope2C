...
def make_combined_frame(gfm, gfm_ps, gfm_AC, rfm, rfm_ps, rfm_AC, behav=None):
    ...
    gfm, gfm_ps, gfm_AC = (
        ...
        gcm.to_rgba(gfm_AC, norm=False)[:, :, :3],
    )
    rfm, rfm_ps, rfm_AC = (
        rcm.to_rgba(rfm, norm=False)[:, :, :3],
        rcm.to_rgba(rfm_ps, norm=False)[:, :, :3],
        rcm.to_rgba(rfm_AC, norm=False)[:, :, :3],
    )
    ovlp, ovlp_ps, ovlp_AC = (
        np.clip(gfm + rfm * 0.75, 0, 1),
        np.clip(gfm_ps + rfm_ps * 0.75, 0, 1),
        np.clip(gfm_AC + rfm_AC, 0, 1),
    )
    gfm, gfm_ps, gfm_AC, rfm, rfm_ps, rfm_AC, ovlp, ovlp_ps, ovlp_AC = (
        annt(gfm, "GCaMP Raw"),
        annt(gfm_ps, "GCaMP Processed"),
        annt(gfm_AC, "GCaMP Segmented"),
        annt(rfm, "tdTomato Raw"),
        annt(rfm_ps, "tdTomato Processed"),
        annt(rfm_AC, "tdTomato Segmented"),
        annt(ovlp, "Overlap"),
        annt(ovlp_ps, "Overlap Processed"),
        annt(ovlp_AC, "Overlap Segmented"),
    )
    out_im = np.concatenate(
        [
            np.concatenate([gfm, rfm, ovlp], axis=1),
            np.concatenate([gfm_ps, rfm_ps, ovlp_ps], axis=1),
            # np.concatenate([gfm_AC, rfm_AC, ovlp_AC], axis=1),
        ],
        axis=0,
    )
...
gRaw = side_int_ds["varr"].sel(**SUBSET)
rRaw = top_int_ds["varr"].sel(**SUBSET)
gPs = side_int_ds["Y_fm_chk"].sel(**SUBSET)
rPs = top_int_ds["Y_fm_chk"].sel(**SUBSET)
gAC = save_minian(
...
out = xr.apply_ufunc(
    ...
    input_core_dims=[["height", "width"]] * 6 + [["height_b", "width_b"]],
